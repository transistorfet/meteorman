...
if (Meteor.isClient) {
    ...
    Template.GameWaiting.helpers({
        ...
        getSlots: function () {
            ...
            return Array(this.data.options.maxslots).fill().map(function (current, index, array) {
                var slot = { };
                slot.slot = index + 1;
                if (index < data.options.computers) {
                    slot.type = 'computer';
                }
                else if (index - data.options.computers < data.users.length) {
                    slot.type = 'user';
                    slot.userId = data.users[index - data.options.computers];
                    slot.name = Meteor.users.findOne({ _id: slot.userId }).username;
                }
                else {
                    slot.type = 'open';
    ...
    Router.route('/play/:gameId', {
        ...
        waitOn: function () {
            ...
            this.subscribe('CurrentGameUsers', this.params.gameId);
        },
    });
