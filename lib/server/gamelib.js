...
if (Meteor.isServer) {
    ...
    Meteor.publish('CurrentGameData', function(gameId) {
        ...
        self.added('CurrentGameData', gameId, getData(gameId));
        self.ready();
    });

    Meteor.publish('CurrentGameUsers', function(gameId) {
        var game = GameState.findOne({ _id: gameId }, { fields: { users: 1 } });
        return Meteor.users.find({ _id: { $in: game.users } }, { fields: { username: 1 } });
    });
    ...
    Meteor.methods({
        ...
        joinGame: function (gameId) {
            ...
            if (!game.started && (game.users.length + 1 + game.options.computers) >= ( game.options.waitall ? game.options.maxslots : gameinfo.minslots ))
